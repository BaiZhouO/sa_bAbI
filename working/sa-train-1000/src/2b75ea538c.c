#include <stdlib.h>                                      // Tag.OTHER
int main()                                               // Tag.OTHER
{                                                        // Tag.OTHER
    int entity_1;                                        // Tag.BODY
    int entity_2;                                        // Tag.BODY
    int entity_8;                                        // Tag.BODY
    char entity_3[74];                                   // Tag.BODY
    char entity_7[58];                                   // Tag.BODY
    int entity_0;                                        // Tag.BODY
    entity_2 = 68;                                       // Tag.BODY
    entity_8 = rand();                                   // Tag.BODY
    entity_0 = 80;                                       // Tag.BODY
    if (entity_8 < entity_0){                            // Tag.BODY
    } else {                                             // Tag.BODY
    entity_8 = 71;                                       // Tag.BODY
    }                                                    // Tag.BODY
    for(entity_1 = 21; entity_1 < entity_8; entity_1++){ // Tag.BODY
    }                                                    // Tag.BODY
    entity_7[entity_1] = '5';                            // Tag.BUFWRITE_COND_UNSAFE
    int *entity_9;                                       // Tag.POINTER_DEC
    entity_9 = &entity_1;                                // Tag.POINTER_DEC
    char entity_6[81];                                   // Tag.BODY
    char *entity_5 = (char *)(entity_9 + 3);             // Tag.INCORRECT_POINTER_SCALING_WEAKNESS
    int entity_4;                                        // Tag.BODY
    entity_4 = 57;                                       // Tag.BODY
    entity_6[entity_4] = '1';                            // Tag.BUFWRITE_TAUT_SAFE
    entity_3[entity_2] = 'W';                            // Tag.BUFWRITE_TAUT_SAFE
    return 0;                                            // Tag.BODY
}                                                        // Tag.OTHER